...
<body>
		...
		<table class="table">
			...
			<tr>
				<td><textarea id="content" name="content"
						style="width: 100%; height: 80px;"><%=dto.getContent()%></textarea></td>
			</tr>
			<tr>
				<td><input type="hidden" id="avata" name="avata"
					value="<%=dto.getAvata()%>"><br> <%
 					for (int i = 1; i <= 10; i++) {%> 
 ...
 %> 
 <script type="text/javascript">
		//db에 저장된 이미지에 .select 적용
		var avata = $("#avata").val();
		$("img").eq(avata-1).addClass("select"); //고른게 1이면 실제로는 0번지 select
